...
ARG=$(ruby -e "puts (0..($n)).to_a.shuffle.join(' ')"); ./push_swap $ARG | ./tests/tests/checker_Mac $ARG
exit 0
fi

if [ $1 == -v2 ];then
ARG=$2; ./push_swap $ARG | ./tests/tests/checker_Mac $ARG
exit 0
fi
...
S=$(./push_swap $ARG | ./tests/checker_Mac $ARG)
if [ $S == "OK" ]; then
	printf "${GREEN}2.[OK] ${DEF_COLOR}";
else
	printf "${RED}2.[KO]${DEF_COLOR}";
...
ARG="2 1 3";
S=$(echo -e "sa" | ./tests/checker_Mac $ARG)
